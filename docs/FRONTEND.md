...
| Concern | Module | Rule |
|---|---|---|
| Money | `lib/formatters.fmtCurrency` | Currency is a property of the **organization** (core_v2 `company.currency_code`, default EUR when unset) and of each profit amount — never of a person, never of the display language (`CURRENCY-ORG`, 24/09/2026). Chosen in **Settings → Preferences** by a manager (owner / admin) only; everyone else sees the picker disabled. The list of offered codes is `config/currencies.js` (`SUPPORTED_CURRENCIES`, which must stay equal to the core_v2 `currency` seed), written by `stores/profile.setCurrency` → RPC `core_v2_set_organization_currency`. Only separators and symbol position follow the locale. Zero conversion — the symbol changes, the stored amounts do not. A quote is the exception: it follows its own billing country. |
