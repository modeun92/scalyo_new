...
// CR-Portfolio: normalizes the contacts array (jsonb). Guarantees exactly one
// is_primary as soon as there is ≥ 1 contact; keeps the old {name,email,role} formats.
...
export const useClientStore = defineStore('clients', () => {
  ...
  // HEALTH-SCALE (25/08): thresholds and effective status = lib/health (single source, /10).
  // Kept here as a store access point for existing consumers.
  ...
  // KPIs computed on ACTIVE clients only (prospects have no realized ARR)
  ...
  // Auto KPIs batch (contract 22/07, R21): derived from real data, never entered by hand.
  // New clients over the rolling period (follows the dashboard's 7/30/90 d selector)
  ...
  // Load per CSM: only on ASSIGNED clients (csm_id set, B-04).
  // No assigned client → null (the dashboard renders "—", never an invented figure).
  ...
  // B-05: real renewals within 30 days (renewal_date entered on the client record)
  // — a date computation on real data, never an invented figure (R21)
  ...
  async function loadClients() {
    ...
    try {
      // HOTFIX CAP-1000 (CL-1000): full pagination — PostgREST caps at 1000
      // rows WITHOUT an error. A STABLE sort is required (created_at desc, id desc: a
      // batch import shares the same created_at). Exact count → truthful counters.
      const { rows, total, truncated: cut } = await fetchAllRows(() =>
        supabase.from('clients').select('*', { count: 'exact' })
          .order('created_at', { ascending: false }).order('id', { ascending: false })
      )
      // dedup by id (concurrent insert between two pages) + ATOMIC swap — never a partial state
      const seen = new Set()
  ...
  async function updateClient(client) {
    lastError.value = null
/* B-05b: local merge in app shape — the old dbToClient({...app}) lost renewalDate/csmId until reload */
  ...
  function dbToClient(r) {
    // HEALTH-SCALE: a real score of 0 stayed 5 with `|| 5` (0 is falsy); null/''/invalid → 5 unchanged
  ...
  async function clientToDb(c) {
    ...
    // CR-9 (B-19): organization_id set at insert/update — activates clients_org_manage
    ...
    // D2 — automatic conversion: a prospect marked "Won" becomes a client
    if (lifecycle === 'prospect' && pipeline_stage === 'won') lifecycle = 'client'
    // a client has no pipeline stage
