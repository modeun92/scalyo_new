<template>
  <div class="auth_page">
    <div class="auth_card">
      <div class="auth_logo"><ScalyoLogo :size="48" /><span class="auth_brand">Scalyo</span></div>
      <h1>{{ t('login_title') }}</h1>

      <!-- Email verified banner -->
      <div v-if="emailVerified" class="verified_banner">
      ...
      <form @submit.prevent="handleLogin" class="auth_form">
        <div class="field_group">
          ...
          <input v-model="email" type="email" required class="field_input" :placeholder="t('login_email_ph')" autocomplete="email" />
        </div>
        <div class="field_group">
          <label>{{ t('login_password') }}</label>
          <input v-model="password" type="password" required class="field_input" placeholder="••••••••" autocomplete="current-password" />
        </div>
...
.auth_card { background-color: var(--bg-card); border-radius: 20px; padding: 48px 40px; width: 100%; max-width: 420px; box-shadow: 0 20px 60px rgba(0,0,0,0.08); }
...
.button_primary:disabled { opacity: 0.65; cursor: not-allowed; }
.button_primary.full { width: 100%; }
.forgot_pw_wrapper { text-align: right; margin-top: -4px; }
...
.auth_legal_footer a { color: #7c3aed; text-decoration: none; }
.auth_legal_footer a:hover { text-decoration: underline; }
</style>
