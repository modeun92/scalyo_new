    ...
    <div v-if="show" class="onboarding_overlay">
      <div class="onboarding_modal">
        ...
        <div class="onboarding_step" v-if="step === 3">
          ...
          <div class="onboarding_navigation">
            <button class="onboarding_button_ghost" @click="step--">←</button>
            <button class="onboarding_button_primary" @click="submit" :disabled="!form.ai_consent || saving">{{ saving ? '...' : t('onboarding_wizard_finish') }}</button>
          </div>
        </div>
      </div>
    </div>
...
// D-14: the questionnaire closes only on a confirmed write (isComplete flips in the store). On a
// failure withWrite has already shown the error toast and the answers stay on screen to retry.
async function submit() {
  saving.value = true
  await profileStore.completeOnboarding({
    role: form.value.role,
    seniority: form.value.seniority,
    aiConsent: form.value.ai_consent,
    analyticsConsent: form.value.analytics_consent,
  })
  saving.value = false
}
</script>
