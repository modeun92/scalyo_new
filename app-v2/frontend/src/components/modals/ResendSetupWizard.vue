...
<div><h3 class="font-semibold text-slate-900 dark:text-white">{{ t('email_studio_resend_wizard_title') }}</h3>
<p class="text-xs text-slate-500">{{ t('email_studio_resend_wizard_subtitle') }}</p></div></div>
...
<h4 class="font-semibold text-slate-900 dark:text-white">{{ t('email_studio_resend_wizard_s1_title') }}</h4>
<p class="text-sm text-slate-600 dark:text-slate-400">{{ t('email_studio_resend_wizard_s1_desc') }}</p>
<div class="bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-900 rounded-lg p-4">
<p class="text-sm font-medium text-indigo-900 dark:text-indigo-200 mb-1">{{ t('email_studio_resend_wizard_s1_free_title') }}</p>
<ul class="text-xs text-indigo-800 dark:text-indigo-300 space-y-1">
<li>{{ t('email_studio_resend_wizard_s1_bullet1') }}</li>
<li>{{ t('email_studio_resend_wizard_s1_bullet2') }}</li>
<li>{{ t('email_studio_resend_wizard_s1_bullet3') }}</li></ul></div>
<div class="flex gap-2">
<a href="https://resend.com/signup" target="_blank" class="flex-1 text-center px-4 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium">{{ t('email_studio_resend_wizard_s1_cta_create') }}</a>
<button @click="step=2" class="flex-1 px-4 py-2.5 rounded-lg border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800">{{ t('email_studio_resend_wizard_s1_cta_have') }}</button>
</div></div>
<div v-if="step===2" class="space-y-4">
<h4 class="font-semibold text-slate-900 dark:text-white">{{ t('email_studio_resend_wizard_s2_title') }}</h4>
<p class="text-sm text-slate-600 dark:text-slate-400">{{ t('email_studio_resend_wizard_s2_desc') }}</p>
<div><label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ t('email_studio_resend_wizard_s2_label') }}</label>
...
<button @click="testKey" :disabled="!canTest||testing" class="flex-1 px-4 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium disabled:opacity-50">{{ testing ? t('email_studio_resend_wizard_s2_testing') : t('email_studio_resend_wizard_s2_cta_test') }}</button>
</div></div>
...
<h4 class="font-semibold text-slate-900 dark:text-white text-lg">{{ t('email_studio_resend_wizard_s3_title') }}</h4>
<p class="text-sm text-slate-600 dark:text-slate-400">{{ t('email_studio_resend_wizard_s3_desc') }}</p>
<button @click="handleDone" class="w-full mt-4 px-4 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium">{{ t('email_studio_resend_wizard_s3_cta_done') }}</button>
</div></div></div></div>
</template>
<script setup>
...
async function testKey() {
  error.value = ''
  if (!canTest.value) { error.value = t('email_studio_resend_wizard_err_format'); return }
  testing.value = true
  try {
    const testResp = await apiFetch('/api/email/test', { api_key: apiKey.value })
    const testData = await testResp.json().catch(() => ({}))
    if (!testResp.ok || !testData?.valid) { error.value = t('email_studio_resend_wizard_err_invalid'); return }
    const saveResp = await apiFetch('/api/email/config', { api_key: apiKey.value })
    if (!saveResp.ok) {
      const saveData = await saveResp.json().catch(() => ({}))
      throw new Error(saveData.error || t('email_studio_resend_wizard_err_generic'))
    }
    apiKey.value = ''
    step.value = 3
  } catch (e) { error.value = e.message || t('email_studio_resend_wizard_err_generic') }
