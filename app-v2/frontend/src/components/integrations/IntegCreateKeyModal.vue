<template>
  <div v-if="open" class="modal_overlay" @click.self="$emit('close')">
    <div class="modal_card">
      <h3>🔑 {{ t('integration_create_key') }}</h3>
      <div class="modal_form">
        <div class="field_group">
          <label>{{ t('integration_key_name') }}</label>
          <input v-model="name" type="text" class="modal_input" placeholder="Ex: HubSpot, Zapier..." />
        </div>
        <div class="field_group">
          <label>{{ t('integration_key_scopes') }}</label>
          <div class="scope_toggles">
            <label class="scope_toggle">
              <input type="checkbox" v-model="scopes" value="read" /> {{ t('integration_scope_read') }}
            </label>
            <label class="scope_toggle">
              <input type="checkbox" v-model="scopes" value="write" /> {{ t('integration_scope_write') }}
            </label>
          </div>
        </div>
      </div>
