<template>
  <SlideOver :open="open" :title="editId ? t('portfolio_edit_title') : t('portfolio_create_title')" @close="$emit('close')">
    <form @submit.prevent="$emit('save')" class="slideover_form">
      ...
      <div class="field_row">
        ...
        <div class="field_group">
          <label>{{ t('portfolio_field_status') }}</label>
      ...
      <div class="fdiv contacts_header">
        <span>{{ t('portfolio_contacts_title') }}</span>
        <button type="button" class="contact_add" @click="addContact">+ {{ t('portfolio_contact_add') }}</button>
      </div>

      <p v-if="!form.contacts.length" class="contact_empty">{{ t('portfolio_contacts_empty') }}</p>

      <div v-for="(ct, i) in form.contacts" :key="i" class="contact_card" :class="{ primary: ct.is_primary }">
        <div class="contact_card_top">
          <button type="button" class="contact_primary" :class="{ on: ct.is_primary }" @click="setPrimary(i)" :title="t('portfolio_contact_primary')">
            ★ {{ ct.is_primary ? t('portfolio_contact_is_primary') : t('portfolio_contact_set_primary') }}
          </button>
          <button type="button" class="contact_remove" @click="removeContact(i)" :title="t('portfolio_contact_remove')">✕</button>
        </div>
        <div class="field_row">
          <div class="field_group">
            <label>{{ t('portfolio_field_contact_name') }}</label>
            <input v-model="ct.name" class="field_input" />
          </div>
          <div class="field_group">
            <label>{{ t('portfolio_field_contact_role') }}</label>
            <input v-model="ct.role" class="field_input" />
          </div>
        </div>
        <div class="field_row">
          <div class="field_group">
            <label>{{ t('portfolio_field_contact_email') }}</label>
            <input v-model="ct.email" type="email" class="field_input" />
          </div>
          <div class="field_group">
            <label>{{ t('portfolio_field_contact_phone') }}</label>
