...
export function downloadPdf(q, laws, billingCountry, t, clientName) {
  ...
  doc.text(q.company || clientName || t('quote_default_company'), 20, 25)
  ...
  doc.setFontSize(14)
  doc.setFont('helvetica', 'bold')
  doc.text(t('quote_title') + ' — ' + q.id, 20, 45)

  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.text(t('quote_create_date') + ' : ' + q.createdAt, 20, 55)
  doc.text(t('quote_field_status') + ' : ' + t('quote_filter_' + q.status), 20, 62)
  ...
  doc.setFontSize(11)
  doc.setFont('helvetica', 'bold')
  doc.text(t('quote_field_title') + ' :', 20, 88)
  doc.setFont('helvetica', 'normal')
  const titleLines = doc.splitTextToSize(q.title, 170)
  doc.text(titleLines, 20, 96)

  doc.setFontSize(10)
  doc.text(t('quote_field_amount') + ' (HT) : ' + pdfMoney(q.amount, l.currency), 20, 110)
  doc.text(t('quote_field_tax') + ' (' + q.tax + '%) : ' + pdfMoney(q.amount * q.tax / 100, l.currency), 20, 118)

  doc.setFontSize(13)
  doc.setFont('helvetica', 'bold')
  doc.text(t('quote_ttc') + ' : ' + pdfMoney(q.amount * (1 + q.tax / 100), l.currency), 20, 130)

  if (q.notes) {
    doc.setFontSize(10)
    doc.setFont('helvetica', 'normal')
    doc.line(20, 138, 190, 138)
    doc.text(t('quote_field_notes') + ' :', 20, 148)
