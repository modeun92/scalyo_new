...
-- reference member). Dropped by decision, so never migrated: is_founding, and the user_profiles
-- columns other than role / seniority. health / nps / churn_risk / renewal_date and the contacts
-- were decided dropped on 20/09 and KEPT on 24/09/2026 (below); arr / mrr become profit rows.
--
-- CORE-V2-PLAN-HOME (24/09/2026): the plan tier is NEVER a column of organization or
-- organization_worker; it belongs to the subscription-information table. A tier column on the
-- organization is a second copy of the subscription, and two copies drift: profiles.plan vs
-- organizations.plan already does, and a member of a paying org is entitled in the UI and 403'd by
-- /api/ai for it (CLAUDE.md, "Traps").
...
-- but a separate thing: a worker points at it directly (organization_worker.role_id), NOT through
...
-- unique (id, organization_id) is the target of the worker's composite foreign key below.
...
-- CORE-V2-COLUMNS (24/09/2026): role and seniority live on organization_worker, not on member.
-- They describe a person's job in ONE organization. On member, which carries no organization,
-- nothing in the database stopped a role_id from pointing at another organization's role; only
-- the sync's own care did. The composite foreign key (role_id, organization_id) refuses it, the
-- same way position_id is checked. No user write policy: the only writers are the questionnaire
-- RPC and the transitional user_profiles mirror (20260924100000). A viewer's answers count too:
-- they describe the job, and a member -> viewer -> member round trip must not lose them now that
-- user_profiles is no longer there to re-read them from. An ENDED worker keeps both as history.
...
begin
  if not exists (
    select 1 from pg_constraint
     where conname = 'organization_worker_role_fkey'
       and conrelid = 'public.organization_worker'::regclass
  ) then
    alter table public.organization_worker
      add constraint organization_worker_role_fkey
      foreign key (role_id, organization_id)
      references public.organization_role (id, organization_id);
  end if;
end $$;
...
create index if not exists idx_organization_worker_role on public.organization_worker (role_id);
...
-- CORE-V2-CLIENT-HEALTH (24/09/2026): the health data stays with the client (decided 24/09/2026,
-- reversing the 20/09 "drop" — the health colours, Satisfaction, the alerts and the MCP risk tools
-- all read it). health is /10 and lib/health is its only scale; no CHECK here, the mirror copies
-- what clients holds rather than failing on a legacy value. health_status is the manual flag
-- clients.status ('critical' / 'watch' / 'todo' / 'healthy'), which can RAISE the colour
-- lib/health.healthStatus computes — not `status`, which on client_group is the ACTIVE / CHURNED
-- lifecycle. created_at feeds "new clients" and the churn-rate denominator; NULL when unknown.
-- ARR / MRR are deliberately NOT columns: ARR = the client group's profit rows dated in the last
-- 12 months, MRR = ARR / 12 (decided 24/09/2026). The old arr becomes one opening profit row (part 2).
alter table public.client_group add column if not exists health numeric;
alter table public.client_group add column if not exists nps numeric;
alter table public.client_group add column if not exists churn_risk numeric;
alter table public.client_group add column if not exists health_status text;
...
-- CORE-V2-CONTACTS: what a contact is FOR this client (clients.contacts[].role — 'Sponsor',
-- 'DSI', free text) and which one is the main contact. Exactly one primary per company when there
-- are contacts — the old store's rule (normalizeContacts), now held by the database.
alter table public.client_group_viewer add column if not exists role text;
alter table public.client_group_viewer add column if not exists is_primary boolean not null default false;
...
revoke all on function public.core_v2_my_company_ids()                                 from public, anon;
revoke all on function public.core_v2_my_contact_ids()                                 from public, anon;
...
grant execute on function public.core_v2_my_company_ids()                              to authenticated;
grant execute on function public.core_v2_my_contact_ids()                              to authenticated;
...
create policy core_v2_company_select on public.company for select to authenticated
  using (id in (select public.core_v2_my_company_ids()));
...
create policy core_v2_company_link_select on public.company_link for select to authenticated
  using (company_id in (select public.core_v2_my_company_ids()));
...
-- ── People: the caller, their colleagues, and the contacts of the companies they can see ──────
...
create policy core_v2_personage_select on public.personage for select to authenticated
  using (
    id in (select public.core_v2_my_colleague_ids())
    or id in (select public.core_v2_my_contact_ids())
  );
...
create policy core_v2_personage_link_select on public.personage_link for select to authenticated
  using (
    personage_id in (select public.core_v2_my_colleague_ids())
    or personage_id in (select public.core_v2_my_contact_ids())
  );
...
-- 12.7 — A worker cannot hold another organization's role. In a transaction you roll back: two
--        organizations, a role in the first, a worker of the second pointed at it. Expect:
--        violates foreign key constraint "organization_worker_role_fkey".
